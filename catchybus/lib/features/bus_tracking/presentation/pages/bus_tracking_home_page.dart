...
class _BusTrackingHomePageState extends ConsumerState<BusTrackingHomePage> {
  ...
  void initState() {
    super.initState();
    _loadCustomMarker();
    _loadAlarms(); // Load persistent alarms
    // Get user's bus number or fallback to default
    final user = ref.read(authProvider).user;

    // Use the passed busNumber if available, otherwise from user, then fallback
  ...
  Set<Polyline> _buildPolylines(BusRoute busRoute) {
    ...
    // Gray polyline for the already-traveled portion
    if (passedPoints.length >= 2) {
      polylines.add(
        Polyline(
          polylineId: const PolylineId('bus_route_passed'),
          points: passedPoints,
          color: const Color(0xFFB0BEC5), // Blue-gray 200
          width: 7,
  ...
  Widget build(BuildContext context) {
    ...
    // Listen to tracking state and update map camera to follow bus
  ...
  Widget _buildStopItem({
    ...
    final bool isStudentAssignedHere = studentStopName != null &&
                ...
                if (!isLast)
                  Expanded(
                    child: isTransitSegment
                        // Bus is on this segment — split line blue/grey
                        ? Stack(
                            ...
                            children: [
                              Column(
                                children: [
                                  Expanded(
                                    flex: (transitProg * 100).round().clamp(
                                      1,
                                      99,
                                    ),
                                    child: Container(
                  ...
                  children: [
                    ...
                    if (isStudentAssignedHere) ...[
                      Container(
                        ...
                        child: Row(
                          ...
                          children: const [
                            Icon(Icons.person_pin_circle, size: 13, color: Colors.black87),
                            SizedBox(width: 4),
                            Text(
                              'Your Stop',
                              style: TextStyle(
                    ...
                    Row(
                      children: [
                        ...
                        if (isNext && !isAtStop && stop.estimatedArrivalMinutes == null)
  ...
  Widget _buildBottomCardContent(
    ...
    // Distance: bus → student's stop (km) via Haversine
    double? stopDistanceKm;
    if (myStop != null) {
      final rawM = Geolocator.distanceBetween(
        busRoute.busPosition.latitude,
        busRoute.busPosition.longitude,
        myStop.latitude,
        myStop.longitude,
      );
      stopDistanceKm = rawM / 1000.0;
    }

    // ETA: prefer per-stop estimatedArrivalMinutes, fallback to route-level
    final int? stopEtaMin = myStop?.estimatedArrivalMinutes;
    ...
    final String distText = studentIsBoarded
        ? 'Safe Journey!'
        : (stopDistanceKm != null
              ? '${stopDistanceKm.toStringAsFixed(1)} km away'
              : '${distance.toStringAsFixed(1)} km away');

    final String arrivalLabel = studentIsBoarded
        ? 'Status'
        : (myStop != null
              ? 'ETA to ${myStop.name.split(' ').take(2).join(' ')}'
              : 'Arriving in');
              ...
              setState(() {
                ...
                _selectedDestination =
                    ...
                    (() {
                      try {
                        return stops.firstWhere(
                          (s) => s.name.toLowerCase().contains('college') ||
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      if (bus.distance != null &&
                                        ...
                                        const SizedBox(width: 8),
                                        Text(
                                          '${bus.distance!.toStringAsFixed(1)} km away',
                                          style: const TextStyle(
                                            fontSize: 12,
                                            color: Colors.grey,
                                          ),
