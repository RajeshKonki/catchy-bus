...
class BusTrackingSocketDataSourceImpl implements BusTrackingRemoteDataSource {
  ...
  Future<BusRouteModel> getBusRoute(String busNumber) async {
    try {
      ...
      return BusRouteModel.fromJson(response.data);
    } catch (e) {
      rethrow;
    }
  }
  ...
  Stream<Map<String, dynamic>> streamTripStatus(String busNumber) {
    ...
    // Also promote any bus_location_update with isTripActive=true as a trip signal
    void onLocationUpdateForTripActive(dynamic data) {
      if (data is Map && data['isTripActive'] == true && !controller.isClosed) {
