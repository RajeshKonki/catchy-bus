...
class _TripTrackingPageState extends ConsumerState<TripTrackingPage> {
  ...
  String? _activeRouteId; // routeId from selected route
  ...
  // ── Route-following GPS simulation (fires when physical GPS is stationary) ──
  Timer? _simTimer; // ticks every 2s to advance the simulated bus
  double _simLat = 0; // current synthetic latitude
  double _simLng = 0; // current synthetic longitude
  int _simStopIdx = 0; // which stop we're currently heading TOWARD
  double _simFraction = 0; // 0.0 = departed from stop, 1.0 = arrived
  bool _simActive = false; // true when physical GPS is stationary
  int _stationaryCount = 0; // consecutive identical GPS readings
  static const double _simSpeedMs = 8.33; // ~30 km/h in m/s
  static const double _simIntervalS = 2.0; // seconds per tick

  /// Get route stops from current provider state
  ...
  void _startRouteSimulation() {
    ...
    _simActive = true;
    _simStopIdx = 1; // head toward stop index 1 first
    _simFraction = 0;
    _simLat = stops[0].latitude;
    _simLng = stops[0].longitude;
    _simTimer?.cancel();
    _simTimer = Timer.periodic(
      const Duration(milliseconds: (2000)), // 2-second ticks
      (_) => _tickSimulation(),
    );
  }
  ...
  void _tickSimulation() {
    ...
    final stops = _routeStops;
    if (_simStopIdx >= stops.length) {
      _simTimer?.cancel();
      return;
    }

    final fromStop = stops[_simStopIdx - 1];
    final toStop = stops[_simStopIdx];
    ...
    // How much fraction to advance per tick
    final step = segmentM > 0 ? (_simSpeedMs * _simIntervalS) / segmentM : 1.0;
    _simFraction = (_simFraction + step).clamp(0.0, 1.0);

    // Interpolate lat/lng
    _simLat =
        fromStop.latitude +
        (_simFraction * (toStop.latitude - fromStop.latitude));
    _simLng =
        fromStop.longitude +
        (_simFraction * (toStop.longitude - fromStop.longitude));

    // Compute compass bearing from fromStop → toStop so the bus icon rotates
    // correctly on the map. Formula: atan2(Δlon, Δlat) converted to 0-360°.
    final dLat = toStop.latitude - fromStop.latitude;
    final dLng = toStop.longitude - fromStop.longitude;
    // atan2 gives angle from north in radians; convert to degrees 0-360
    final bearingRad = math.atan2(dLng, dLat);
    final bearingDeg = (bearingRad * 180.0 / math.pi + 360.0) % 360.0;

    // Send synthetic position with correct bearing to socket and local UI
    ...
    // Advance to next stop when we arrive
    if (_simFraction >= 1.0) {
      _simStopIdx++;
      _simFraction = 0;
    }
    ...
    if (user?.busNumber != null) {
      if (mounted) {
        setState(() {
          ...
          _activeRouteId = widget.routeId; // from RouteSelectionPage
        });
      }
  ...
  Future<void> _startTracking() async {
    ...
    // Notify server that a trip has started, including routeId + driver location for geofence
    ...
    _socketService.on('students_list', (data) {
      Log.i(
        'Received students list: ${data['students'].length} students for ${data['stopName']}',
      );
      if (mounted) {
        setState(() {
    ...
    try {
      ...
      Log.i(
        'Initial starting location sent: ${initialPosition.latitude}, ${initialPosition.longitude}',
      );
    } catch (e) {
    ...
    if (defaultTargetPlatform == TargetPlatform.android) {
      locationSettings = AndroidSettings(
        ...
        distanceFilter: 2, // Tightened from 3m
        intervalDuration: const Duration(milliseconds: 1000), // Every 1s
        // Use high-priority foreground service to prevent OS throttling
    ...
    if (mounted) {
      ...
      _refreshTimer = Timer.periodic(const Duration(seconds: 5), (_) async {
        try {
          ...
          // Measure actual distance moved in meters (immune to degree-unit noise)
          final movedM = _lastPosition == null
              ? 0.0
          ...
          // < 10m  → treat as stationary (covers typical GPS noise of 1-5m)
          // > 50m  → treat as genuine real-world movement (stops simulation)
          // 10-50m → grey zone: keep current mode (hysteresis)
          final bool nowStationary = movedM < 10.0;
          final bool nowMoving = movedM > 50.0;

          if (nowStationary) {
            _stationaryCount++;
            if (_stationaryCount >= 2 && !_simActive) {
              Log.w(
                'GPS stationary (${movedM.toStringAsFixed(1)}m) — starting route simulation',
              );
              _startRouteSimulation();
            }
          } else if (nowMoving) {
            // Genuine GPS movement — use real position, stop simulation
            if (_simActive) {
              Log.i(
                'GPS moved ${movedM.toStringAsFixed(1)}m — stopping route simulation',
              );
            ...
            Log.d(
              '5s refresh (real GPS moved ${movedM.toStringAsFixed(1)}m): ${position.latitude}, ${position.longitude}',
            );
          }
          // 10-50m = grey zone, keep current mode unchanged
        } catch (e) {
          ...
          children: [
            ...
            Padding(
              ...
              child: Row(
                children: [
                  Icon(
                    (_isTracking || (busRoute?.isTripActive ?? false))
                        ? Icons.sensors
                        : Icons.sensors_off,
                    color: Colors.white,
                    size: 32,
              ...
              child: Row(
                children: [
                  ...
                  _buildStatCard(
                    ...
                    onTap: () {
                      if (busRoute != null) {
                        final boardedStudents = busRoute.students
                            .where((s) => s['isBoarded'] == true)
                            .toList();
                        _showStudentListModal(
                          context,
                  ...
                  _buildStatCard(
                    ...
                    onTap: () {
                      if (busRoute != null) {
                        final pendingStudents = busRoute.students
                            .where(
                              (s) =>
                                  s['isBoarded'] == false ||
                                  s['isBoarded'] == null,
                      ...
                      if (busRoute != null)
                        Positioned(
                          ...
                          child: Container(
                            ...
                            child: IconButton(
                              onPressed: () =>
                                  _showTripCompletedModal(context, busRoute),
                              icon: const Icon(
                                Icons.check,
                                color: Colors.white,
                                size: 30,
  ...
  Widget _buildRouteTimeline() {
    final busState = ref.watch(busTrackingProvider);
    // Capture the outer (Scaffold-level) context so modal callbacks work
    // even after async gaps in helper functions.
    ...
    return busState.when(
      ...
      loaded: (busRoute) {
        ...
        // Update incremental tracking (schedule after build)
        if (transitProg > 0) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            setState(() => _updateTransitPrev(transitFrom, transitProg));
          });
        }
        ...
        return Column(
          children: List.generate(stopsData.length, (index) {
            ...
            final isFuture =
                !isPassed && !isSkipped && !isAtStop && !isTransitSegment;
                        ...
                        if (!isLast)
                          Expanded(
                            child: isTransitSegment
                                // Active transit segment: animate smoothly linearly over 3 seconds
                                ? TweenAnimationBuilder<double>(
                                ...
                                // Passed or future segment: solid line
                                : Container(
                                    width: isPassed ? 8 : 4,
                                      ...
                                      color: () {
                                        if (isInTransit) {
                                          return index < transitFrom
                                              ? AppColors.navigationBlue
                                              : const Color(0xFFE0E0E0);
                                        } else {
                                          return (atStopIndex != null &&
                                                  index < atStopIndex)
                                              ? AppColors.navigationBlue
                                              : const Color(0xFFE0E0E0);
                                        }
                        ...
                        Row(
                          children: [
                            if (isAtStop || isFuture) ...[
                              ElevatedButton(
                                onPressed: () =>
                                    _showSkipStopModal(context, stop.name),
                            ...
                            OutlinedButton(
                              onPressed: () =>
                                  _showAttendanceModal(outerContext, stop.name),
                          ...
                          onPressed: _selectedSkipReason == null
                              ...
                              : () {
                                  _socketService.skipStop(
                                    ...
                                    stopName: stopName,
                                    reason:
                                        _selectedSkipReason ?? 'Manual Skip',
                                  );

                                  ref
                                      .read(busTrackingProvider.notifier)
                                      .markStopSkipped(stopName);

                                  Navigator.pop(modalContext);
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text(
                                        'Stop "$stopName" skipped successfully',
                                      ),
                  ...
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: ElevatedButton(
                          onPressed:
                              (_selectedCancelReason == null || isCancelling)
                              ? null
                              : () {
  ...
  void _showAttendanceModal(BuildContext context, String stopName) async {
    ...
    if (stop != null) {
      ...
      // Guard: widget may have been disposed during the await
      if (!mounted) return;
      final distance = Geolocator.distanceBetween(
        currentPos.latitude,
      ...
      if (distance > 200 && context.mounted) {
        UIHelpers.showWarningTooltip(
          context,
          'Warning: You are ${(distance / 1000).toStringAsFixed(2)} km from $stopName. Please ensure you are at the correct stop.',
        );
    ...
    // Fetch students via HTTP
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      await ref
          .read(busTrackingProvider.notifier)
          .fetchStudentsForStop(
            _busId,
            stopName,
            onData: (students) {
              if (mounted) {
                setState(() {
                  _currentStopStudents = List<Map<String, dynamic>>.from(
                    students,
                  );
                  _isLoadingStudents = false;
                });
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      OutlinedButton.icon(
                        onPressed: () async {
                          final result = await context.push<QrScanResult>(
                            AppRouter.attendanceQrScanner,
                          );
                          if (result != null && mounted) {
                            ...
                            // Immediately update local state to reflect in stats on top
                            ref.read(busTrackingProvider.notifier).markStudentAsBoarded({
                              'id': result.studentId,
                              'name': result.studentName,
                              'stopName': stopName,
                            });

                            // Immediately emit markAttendance so the student gets push notified instantly
                              ...
                              SnackBar(
                                content: Row(
                                  children: [
                                    ...
                                    const SizedBox(width: 8),
                                    Text(
                                      '${result.studentName} added to attendance',
                                    ),
                                  ],
                                ),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        ...
                        final boardedStudentIds = _selectedStudents
                            .where(
                              (id) => _currentStopStudents.any(
                                (s) => s['id'] == id,
                              ),
  ...
  void _showTripCompletedModal(BuildContext context, BusRoute busRoute) {
    ...
    final completedStops = busRoute.stops
        .where((s) => s.type == StopType.passedStop)
        .length;
    final skippedStops = busRoute.stops
        .where((s) => s.type == StopType.skippedStop)
        .length;

    final totalStudents = busRoute.stops.fold(
      0,
      (sum, s) => sum + (s.studentCount ?? 0),
    );
    final onboardedStudents = busRoute.stops.fold(
      0,
      (sum, s) => sum + (s.boardedStudentCount ?? 0),
    );
                  ...
                  Expanded(
                    child: ElevatedButton(
                      onPressed: () {
                        ...
                        // Grab latest position from state (driverLat/Lng were provided in widget, but we've tracked new locs)
                        double? currentLat = widget.driverLat;
                        double? currentLng = widget.driverLng;

                        // We can also pull from the last _updateUserLocation
                        // However, we just grab current from geolocator to be safe:
                        Geolocator.getCurrentPosition()
                            ...
                            .catchError((_) {
                              final summary = TripSummary(
                                ...
                                busId: _busId,
                                endTime: now,
                                driverLat: currentLat, // fallback to initial
